...
def func_subtract(img_new, img_old):
    img_map = np.sum(np.abs(img_new.astype(np.int32) - img_old.astype(np.int32)), axis=-1) / 3
    thres8 = 255 - np.zeros(img_map.shape, dtype=np.uint8)
    thres8[img_map > 30] = 0
    kernel = cv2.getStructuringElement(cv2.MORPH_CROSS ,(3,3))
    opened = cv2.morphologyEx(thres8,cv2.MORPH_OPEN,kernel, iterations=1)
    blur = cv2.medianBlur(opened, 3)
...
def cropping(proced_img, index):
    ...
    ma = -1
    mapos = -1
    for i in range(CHAR_WIDTH * index, imwidth):
...
def gen_images(img, norm=True):
    ...
    for i, index in enumerate((0, 4, 8, 12)):
        ...
        if index == 0:
            ...
            new_img = cropping(new_img, index // 4 + 1)
        else:
            img.seek(index - 1)
            last_img = Image.new("RGB", img.size)
            last_img.paste(img)
